...
class Comment{
	...
	private function updateProductPath($product,$version){
		...
		$db->run("UPDATE webhelp SET value='".__BASE_URL__."' WHERE parameter='path' AND product='".$product."' AND version='".$version."';");				
		$db->Close();
	}
	/**
	...
	function isAnonymous($idUser, $name){
		return ($idUser==1 || $name =='Anonymous');
	}
			...
			while($db->MoveNext()){
				...
				if (($db->Field('userId')==$idUser && $db->Field('state')=='new')||$isForAdmin || $db->Field('state')=='approved'){
					...
					if (($idUser==$db->Field('userId') && $db->Field('state')=='new') && !$isForAdmin){
						...
						if (!$this->isAnonymous($idUser,$db->Field('name'))){
					...
					$toReturn.="</div>";
					if ($this->isAnonymous($idUser,$db->Field('name')) && $mustModerate){
